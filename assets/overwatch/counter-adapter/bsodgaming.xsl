...
<xsl:stylesheet version="1.0"
	...
	<xsl:variable name="sourceTable" select="$source//*[@class='td-post-content']/table"/>
	
	<func:function name="lio:calculate-counter">
		<xsl:param name="thisHero"/>
		<xsl:param name="thatHero"/>
		
		<xsl:variable name="goodList" select="$sourceTable//td[count(preceding-sibling::*) = 1]"/>
		<xsl:variable name="badList" select="$sourceTable//td[count(preceding-sibling::*) = 2]"/>
		
		<xsl:choose>
			<xsl:when test="$goodList[lio:equals((preceding::td[b])[last()], $thisHero)][lio:equals(., $thatHero)]">
				<func:result select="1"/>
			</xsl:when>
			<xsl:when test="$badList[lio:equals((preceding::td[b])[last()], $thisHero)][lio:equals(., $thatHero)]">
				<func:result select="-1"/>
			</xsl:when>
			<xsl:otherwise>
