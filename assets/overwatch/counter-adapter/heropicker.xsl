...
<xsl:stylesheet version="1.0"
	...
	<func:function name="lio:calculate-counter">
		...
		<xsl:variable name="dataRow" select="$sourceTable/tbody/tr[lio:equals(td[1], $thisHero)]/*"/>
		
		<xsl:for-each select="$headRow">
			<xsl:variable name="i" select="position()"/>
			<xsl:if test="lio:equals(., $thatHero)">
				<func:result select="lio:translate-counter(normalize-space($dataRow[$i]/@class))"/>
			</xsl:if>
		</xsl:for-each>
	</func:function>
	...
	<func:function name="lio:translate-counter">
		...
		<xsl:variable name="translation" select="document('')//lio:translation[@key = $key]"/>
		<xsl:choose>
	...
	<lio:translation key="counter-data counter-good counter-strong" val="2"/>
	<lio:translation key="counter-data counter-good counter-medium" val="1"/>
	<lio:translation key="counter-data same-hero" val="0"/>
	<lio:translation key="counter-data" val="0"/>
	<lio:translation key="counter-data counter-bad counter-medium" val="-1"/>
	<lio:translation key="counter-data counter-bad counter-strong" val="-2"/>
	<lio:translation key="" val="HERO NOT FOUND OR SOME OTHER ERROR, HELP"/>
</xsl:stylesheet>
